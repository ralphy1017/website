...
Fetches publications for Rafael Ortiz III from NASA ADS using ORCID.
Writes publications.json to the repo root for the website to consume.
...
def fetch_papers(orcid: str, token: str) -> list[dict]:
    if not token:
        ...
        print("Get your free token at: https://ui.adsabs.harvard.edu/user/settings/token", file=sys.stderr)
        sys.exit(1)
    ...
    print(f"Fetched {len(docs)} papers from ADS.")
    return docs
...
def build_output(docs: list[dict]) -> dict:
    ...
    for d in docs:
        ...
        papers.append({
            ...
            "refereed": is_refereed(d.get("doctype") or ""),
    ...
    stats = {
        ...
        "first_author_papers": sum(1 for p in papers if p["first_author"]),
...
def main():
    ...
    print(f"   h-index (1st)   : {s['h_index_first_author']}")
    print(f"\n✅ Written to: {out_path}")
